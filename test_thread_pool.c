...
static int CreateNewWorker(stThreadPool* pool)
{
  sigset_t oset;
  int error;
  pthread_sigmask(SIG_SETMASK, &global_fillset, &oset);
  error = pthread_create(NULL, &pool->thrd_attr, MasterWorkerFunction, pool);
...
int CreateJobQueue(stThreadPool* pool, 
                   ...
                   void* arg)
{
   stJob* job = calloc(1, sizeof(stJob));
   job->next_job = NULL;
   job->job_func = func;
   job->job_arg = arg;
   int ret_code = 0;
	                       ...
	                       pthread_attr_t* attr)
{
 /* Initialize global signal set global_fillset with all signals*/ 
   ...
   stThreadPool* pool = calloc(1, sizeof(stThreadPool));
